...
// --- multioauth commands: unified multi-provider OAuth account management ---
...
cli
  .command(
    'multioauth anthropic current',
    'Show the current Anthropic OAuth account being used',
  )
  .action(async () => {
...
cli
  .command(
    'multioauth anthropic remove <indexOrEmail>',
    'Remove an Anthropic OAuth account from the rotation pool',
  )
  .action(async (indexOrEmail: string) => {
    ...
    if (resolvedIndex < 0) {
      cliLogger.error(
        'Usage: kimaki multioauth anthropic remove <index-or-email>',
      )
      process.exit(EXIT_NO_RESTART)
    }
...
cli
  .command(
    'multioauth openai list',
    'List stored OpenAI OAuth accounts used for automatic rotation',
  )
  .action(async () => {
...
cli
  .command(
    'multioauth openai current',
    'Show the current OpenAI OAuth account being used',
  )
  .action(async () => {
...
cli
  .command(
    'multioauth openai remove <indexOrEmail>',
    'Remove an OpenAI OAuth account from the rotation pool',
  )
  .action(async (indexOrEmail: string) => {
    const value = Number(indexOrEmail)
    const store = await loadOpenAIAccountStore()
    ...
    if (resolvedIndex < 0) {
      cliLogger.error(
        'Usage: kimaki multioauth openai remove <index-or-email>',
      )
      process.exit(EXIT_NO_RESTART)
    }
...
cli
  .command(
    'multioauth openai check',
    'Test all OpenAI OAuth accounts for usage limits',
  )
  .action(async () => {
    const store = await loadOpenAIAccountStore()
    if (store.accounts.length === 0) {
    ...
    const { saveOpenAIAccountStore } = await import('./openai-auth-state.js')
